...
def train(
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=5, num_training_steps=epochs * len(train_data)
    )
    ...
    for epoch in range(epochs):
        ...
        if if_validation is True:
            ...
            for batch in test_data:
                ...
                # print(logits)
                # print(label_ids)
                tmp_eval_accuracy = flat_accuracy(logits, label_ids, mask_ids)
            ...
            print(
                ...
                {eval_accuracy / eval_steps_per_epoch}"
            )
            print("-----------------------")

    # 儲存validation結果
    if if_validation is True and save_validation is True:
        csv_file = open("comapre2.csv", "w", encoding="utf-8", newline="")
    ...
    # 儲存模型
    torch.save(model, "./pickle/model_v3.pkl")
